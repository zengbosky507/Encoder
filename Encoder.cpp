...
//初始化
Encoder::Encoder(uint8_t Vpin, uint8_t Dpin)
{
	V_pin = Vpin;
	D_pin = Dpin;
	pinMode(V_pin, INPUT);              //计数输入引脚
	pinMode(D_pin, INPUT);              //方向输入引脚
	attachInterrupt(digitalPinToInterrupt(V_pin), ISR0, LOW);           //中断0调用函数blink0
	instance0 = this;
	attachInterrupt(digitalPinToInterrupt(D_pin), ISR1, LOW);           //中断1调用函数blink1
	instance1 = this;
	time = millis();                   //周期时间1s
	timeV = millis();				   //V 时间初值
